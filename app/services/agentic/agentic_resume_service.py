...
async def save_resume_pdf(user_data: Dict[str, Any], output_path: str, cover_letter: Optional[str] = None) -> str:
    """이력서와 자기소개서를 PDF로 저장"""
    try:
        # HTML 템플릿 생성
        html_content = generate_resume_html(user_data, cover_letter)
...
def generate_resume_html(user_data: Dict[str, Any], cover_letter: Optional[str] = None) -> str:
    """이력서와 자기소개서 HTML 생성"""
    html = f"""
    <!DOCTYPE html>
    <html lang="ko">
