...
def get_langchain_llm(is_lightweight: bool = True):
    ...
    if provider == LLMProvider.OPENAI:
        if is_lightweight:
            return ChatOpenAI(
                api_key=settings.LIGHTWEIGHT_OPENAI_API_KEY,
                model=settings.LIGHTWEIGHT_OPENAI_MODEL,
                temperature=0.7
            )
        else:
            return ChatOpenAI(
                api_key=settings.HIGH_PERFORMANCE_OPENAI_API_KEY,
                model=settings.HIGH_PERFORMANCE_OPENAI_MODEL,
                temperature=0.7
            )
    elif provider == LLMProvider.GROQ:
        if is_lightweight:
            return ChatGroq(
                ...
                model=settings.GROQ_LIGHTWEIGHT_MODEL,
                temperature=0.7
            )
        else:
            return ChatGroq(
                api_key=settings.GROQ_API_KEY,
                model=settings.GROQ_HIGHPERFORMANCE_MODEL,
                temperature=0.7
            )
    elif provider == LLMProvider.OLLAMA:
        if is_lightweight:
            model = settings.LIGHTWEIGHT_OLLAMA_MODEL
            base_url = settings.LIGHTWEIGHT_OLLAMA_URL
        else:
            model = settings.HIGH_PERFORMANCE_OLLAMA_MODEL
            base_url = settings.HIGH_PERFORMANCE_OLLAMA_URL
